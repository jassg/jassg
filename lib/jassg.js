...
Jassg.prototype.stats = function() {
	...
	this.createContentTree(function() {
		...
		//Print the urls
		var resultTable = new Table({ head: ['File', 'URL', 'Template', 'Format'] });
		function printUrlsForChild(node) {
			if(node.url != '') {
				resultTable.push([node.path+node.name, node.url, node.template, node.format]);
			}
			for(var i in node.childs) {
...
Jassg.prototype.createContentTree = function(callback) {
	...
	this.contents = new Node('FOLDER', '', '.', null, null);
	this.contents.jassg = this;
